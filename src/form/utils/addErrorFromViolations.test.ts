...
describe("addErrorFromViolations", () => {
  it("efface toutes les violations existantes avant d'appliquer les nouvelles", () => {
  ...
  it("mappe les violations de l'API sur les inputs correspondants", () => {
    const form = createForm({
      inputs: {
        email: { value: "" },
        name: { value: "" },
  ...
  it("ignore les violations sur des propertyPath inconnus", () => {
    const form = createForm({
      inputs: {
        name: { value: "Alice" },
      },
  ...
  it("n'ajoute pas d'erreur globale si propertyPath est vide (retour précoce sur !propertyPath)", () => {
    // Note : dans l'implémentation actuelle, `if (!propertyPath) return` empêche
    ...
    const apiError = {
      violations: [{ propertyPath: "", message: "Erreur générale" }],
    } as any

    const result = addErrorFromViolations(form, apiError)

    // La violation est silencieusement ignorée — aucune erreur n'est ajoutée
    expect(result.errors ?? []).toHaveLength(0)
  })

  it("retourne le formulaire inchangé si apiError n'a pas de violations", () => {
    const form = createForm({
      inputs: {
  ...
  it("peut accumuler plusieurs violations sur le même champ", () => {
    const form = createForm({
      inputs: {
        password: { value: "123" },
      },
