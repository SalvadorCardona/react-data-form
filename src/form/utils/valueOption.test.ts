...
describe("valueOption utils", () => {
  test("valueOptionMapper mappe correctement label, value et original", () => {
    const data = [
  ...
  test("valueOptionFromJsonLdCollection utilise name et @id", () => {
    const data = [
      { "@id": "id1", name: "Nom 1" },
      { "@id": "id2", name: "Nom 2" },
    ] as any[]
  ...
  test("valueOptionFromArray convertit les items en options", () => {
  ...
  test("getValueOptionFromValue retourne l option correspondante", () => {
    const opts = [
      { label: "One", value: 1 },
      { label: "Two", value: 2 },
    ] as any[]
  ...
  test("groupByGroup regroupe par group avec default si non défini", () => {
    const opts = [
