...
const MenuBar = ({ editor }: { editor: Editor | null }) => {
  ...
  const ToolbarButton = ({
    ...
    <TooltipProvider>
      <Tooltip>
        {/* `render` fusionne le déclencheur avec le Toggle : imbriqué, on
    ...
    <div className="border-b border-border bg-muted/30 backdrop-blur-sm rounded-t-lg px-3 py-2 flex flex-wrap items-center gap-1">
      ...
      <div className="flex items-center gap-0.5">
        <TooltipProvider>
          <Tooltip>
            <Popover>
              {/* Les deux déclencheurs et le bouton sont fusionnés en un seul
                  élément : imbriqués, ils produiraient trois <button>
                  emboîtés. */}
              <TooltipTrigger
                render={
                  <PopoverTrigger
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: "Commencez à écrire quelque chose de génial...",
      }),
      Link.configure({
        openOnClick: false,
        HTMLAttributes: {
